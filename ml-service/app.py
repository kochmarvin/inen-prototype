...
# Allow overriding the model location via env var so the service can be run
# either next to a copy of best.pt or pointed at the original file.
...
async def lifespan(_app: FastAPI):
    ...
    state["model"] = model
    state["names"] = model.names if isinstance(model.names, dict) else {i: n for i, n in enumerate(model.names)}
